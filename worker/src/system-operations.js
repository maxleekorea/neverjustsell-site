...
const BEGIN_CUSTOMER_CANCELLED_E2E_REFUND_OPERATION = "begin_payment_e2e_customer_refund";
...
async function acceptPaymentE2ECustomerCancellation(env, row) {
  let payload = {};
  try { payload = JSON.parse(String(row.payload_json || "{}")); } catch {}
  ...
  const date = String(payload.date || "").trim();
  const expectedOrderId = String(payload.order_id || "").trim();
  if (productNo !== 13 || date !== "2026-09-25" || expectedOrderId !== "20260925-0000013") {
    throw new Error("Cancellation acceptance is restricted to the dedicated payment E2E order");
  }
  ...
  const alreadyAccepted = new Set(["C10","C34","C35","C36","C40","C41","C42","C43"]).has(itemStatus);
  if (alreadyAccepted) {
    return {
      operation: ACCEPT_CUSTOMER_CANCELLED_E2E_OPERATION,
  ...
  if (itemStatus !== "C00") {
    throw new Error("Dedicated payment E2E order is not in customer cancellation request state");
  }

  const [entitlement, enrollment] = await Promise.all([
    env.COURSE_DB.prepare(
      "SELECT status,member_id,source_order_id FROM course_entitlements " +
      "WHERE course_id='system-check-paid-course' AND source_order_id=? LIMIT 1"
    ).bind(expectedOrderId).first(),
    env.COURSE_DB.prepare(
      "SELECT status,member_id,source_order_id FROM program_enrollments " +
      "WHERE run_id='system-check-payment-program-run' AND source_order_id=? LIMIT 1"
    ).bind(expectedOrderId).first()
  ]);
  if (entitlement?.status !== "revoked" || enrollment?.status !== "withdrawn") {
    throw new Error("Refusing cancellation acceptance until content access is fully revoked");
  }
  if (String(entitlement?.member_id || "") !== String(enrollment?.member_id || "")) {
  ...
  await cafe24AdminRequest("/orders/" + encodeURIComponent(expectedOrderId) + "/cancellation", env, {
    ...
    body: {
      ...
      add_memo_too: "T",
      claim_reason_type: "I",
      reason: "NEVER JUST SELL 고객 취소 신청 접수 E2E",
      items: [{
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    if (observedStatus !== "C00") break;
    await sleep(1200);
  }

  if (!new Set(["C10","C34","C35","C36","C40","C41","C42","C43"]).has(observedStatus)) {
    throw new Error("Cafe24 cancellation acceptance was not observed");
  }
...
async function advancePaymentE2EToAwaitingRefund(env, row) {
  let payload = {};
  try { payload = JSON.parse(String(row.payload_json || "{}")); } catch {}
  ...
  if (expectedOrderId !== "20260925-0000013" || productNo !== 13 || date !== "2026-09-25") {
    throw new Error("Awaiting-refund transition is restricted to the dedicated payment E2E order");
  }
  ...
  if (awaitingStatuses.has(itemStatus)) {
    return {
      operation: ADVANCE_CUSTOMER_CANCELLED_E2E_AWAITING_REFUND_OPERATION,
      order_id: expectedOrderId,
      awaiting_refund: true,
      already_awaiting_refund: true,
      item_order_status: itemStatus
    };
  }
  if (new Set(["C35","C40"]).has(itemStatus)) {
    throw new Error("Refusing awaiting-refund transition because Cafe24 already reports refund completion");
  }
  if (itemStatus !== "C10") {
  ...
  if (entitlement?.status !== "revoked" || enrollment?.status !== "withdrawn") {
    throw new Error("Refusing awaiting-refund transition until content access is fully revoked");
  }
  ...
  if (!claimCode) throw new Error("Cafe24 cancellation claim code is missing");

  await cafe24AdminRequest(
    "/orders/" + encodeURIComponent(expectedOrderId) + "/cancellation/" + encodeURIComponent(claimCode),
    env,
    {
      method: "PUT",
      body: {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const refreshedItem = (Array.isArray(refreshedOrder?.items) ? refreshedOrder.items : [])
      .find((candidate) => Number(candidate?.product_no || 0) === 13) || null;
    observedStatus = String(refreshedItem?.order_status || refreshedOrder?.order_status || "").trim();
    if (observedStatus !== "C10") break;
    await sleep(1200);
  }

  if (!awaitingStatuses.has(observedStatus)) {
    throw new Error("Cafe24 awaiting-refund cancellation state was not observed");
  }
  ...
  return {
    ...
    awaiting_refund: true,
    already_awaiting_refund: false,
    item_order_status: observedStatus
  };
}


async function beginPaymentE2ECustomerRefund(env, row) {
  let payload = {};
  try { payload = JSON.parse(String(row.payload_json || "{}")); } catch {}
  ...
  const date = String(payload.date || "").trim();
  if (expectedOrderId !== "20260925-0000013" || productNo !== 13 || date !== "2026-09-25") {
    throw new Error("Refund-pending transition is restricted to the dedicated payment E2E order");
  }
  ...
  if (awaitingStatuses.has(itemStatus)) {
    return {
      ...
      order_id: expectedOrderId,
      refund_pending: true,
      already_refund_pending: true,
      item_order_status: itemStatus
    };
  }
  if (new Set(["C35","C40"]).has(itemStatus)) {
    throw new Error("Refusing refund-pending transition because Cafe24 already reports refund completion");
  }
  if (itemStatus !== "C10") {
    throw new Error("Dedicated payment E2E order is not in accepted cancellation state");
  }
  ...
  if (entitlement?.status !== "revoked" || enrollment?.status !== "withdrawn") {
    throw new Error("Refusing refund processing until content access is fully revoked");
  }

  const itemCode = String(item?.order_item_code || "").trim();
  const quantity = Number(item?.quantity ?? item?.order_quantity ?? 1);
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    if (new Set(["C35","C40"]).has(observedStatus)) {
      throw new Error("Cafe24 unexpectedly reported refund completion before cash settlement");
    }
    await sleep(1200);
  }
    ...
    try {
      ...
      const detail = row.operation_type === OPEN_E2E_OPERATION
        ...
        : row.operation_type === RECONCILE_E2E_ORDER_OPERATION
          ...
          : row.operation_type === ACCEPT_CUSTOMER_CANCELLED_E2E_OPERATION
          ...
          : row.operation_type === BEGIN_CUSTOMER_CANCELLED_E2E_REFUND_OPERATION
            ? await beginPaymentE2ECustomerRefund(env, row)
