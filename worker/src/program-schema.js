...
const MIGRATION_0045 = String.raw`
INSERT OR IGNORE INTO system_operations (
  id,operation_type,status,payload_json
) VALUES (
...
async function ensureInternal(db) {
  ...
  if (!(await migrationApplied(db, "0045_accept_payment_e2e_customer_cancellation.sql"))) {
  ...
  if (!(await migrationApplied(db, "0046_persist_separated_refund_processing.sql"))) {
    await executeStatements(db, MIGRATION_0046);
    await markMigration(db, "0046_persist_separated_refund_processing.sql");
    applied.push("0046_persist_separated_refund_processing.sql");
  }
